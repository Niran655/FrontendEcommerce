...
const InShopPage = () => {
  ...
  const [createSale, { loading: saleLoading }] = useMutation(CREATE_SALE, {
    onCompleted: (data) => {
      const storageKey = `cart_shop_${id}`;
      setCart([]);
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      try {
        ...
        const validCartItems = cartData.filter((item) => item.shopId === id);
  ...
  const shopInformation = shopData?.shop || [];
  ...
  const addToCart = useCallback(
    (product) => {
      if (cart.length > 0 && cart[0].shopId !== id) {
        alert(
          "Please complete your current order before adding items from another shop"
        );
        return;
      }
      ...
      if (existingItem) {
        ...
        setCart([
          ...cart,
          {
            ...
            quantity: 1,
            shopId: id,
            shopName: shopInformation.shopName,
          },
        ]);
      }
    },
    [cart, id, shopInformation.shopName]
  );
  ...
  return (
    <Box>
        ...
        <Stack direction={"row"} maxWidth={400}>
          <Box maxWidth={150}>
            <CardMedia
              ...
              alt={shopInformation.name}
              sx={{ objectFit: "cover", height: "150px" }}
            />
          </Box>
          <Typography class="shop-info-shopName">
      ...
      <Checkout  open={paymentDialogOpen} cartItems={cart} shopId={id}  onClose={()=>setPaymentDialogOpen(false)}  />
